...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    // Call Resend API directly using native fetch
    const resendResponse = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `Odhlášení z newsletteru: ${email}`,
        text: `Uživatel se odhlásil z odběru newsletteru: ${email}`,
      }),
    });
    ...
    if (!resendResponse.ok) {
      ...
      return {
        ...
        body: JSON.stringify({ error: resendData.message || "Failed to process unsubscribe request via Resend API." }),
      };
    }
    ...
    console.error("unsubscribe Serverless Function Catch Block Error:", err);
    return {
      statusCode: 500,
      headers: { 
        "Content-Type": "application/json",
      ...
      body: JSON.stringify({
        error: err instanceof Error ? err.message : "Internal Server Error during unsubscribe execution.",
      }),
    };
  }
};
